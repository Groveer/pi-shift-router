...
/** Snapshot of the cost-telemetry view (SPEC §9 — Cost telemetry deep view). */
 ...
 * (priority 1)?" — the natural pre-router setup. Pure when `store` is
 * omitted or the Smart tier has no pricing (baseline stays 0 / null).
 */
...
export function formatStats(
  ...
  const s = computeStats(state, config, now, store);
  const lines: string[] = [];
  ...
  if (c.baselineModel) {
    ...
    lines.push(`  baseline: unavailable (Smart tier unconfigured or no pricing in models-store.json)`);
  }
 ...
 * highest-priority model (SPEC §4.3); when the fast tier is empty it falls
 * back to the cheapest globally-priced model, which we can't resolve here
 * without the store — so we show the configured fast chain instead.
 */
export function judgeModelDisplay(config: ShiftRouterConfig): string {
  const fastModels = [...(config.tiers?.fast?.models ?? [])].sort(
    (a, b) => a.priority - b.priority,
  );
  if (fastModels.length === 0) return "(fast tier empty — cheapest global)";
